...
# Molecular weight of the exhaust gas (kg/kmol) (only used to calculate R,
# and hence cp)
molecular_weight = 21.627
...
nozzle = bam.Nozzle.from_engine_components(
    perfect_gas, chamber, ex.p_amb, type="rao", length_fraction=0.8)
...
# Estimate apogee based on apprpoximate Martlet 4 vehicle mass and cross
# sectional area
...
# Run an optimisation program to change the nozzle area ratio, to maximise
# the apogee obtained (I'm not sure if this is working correctly right
# now).
white_dwarf.optimise_for_apogee(
    dry_mass=60,
    propellant_mass=50,
    cross_sectional_area=0.03)
