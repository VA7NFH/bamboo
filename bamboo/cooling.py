...
class EngineGeometry:
    def __init__(self, nozzle, chamber_length, chamber_area,
                 wall_thickness, geometry="auto"):
        self.nozzle = nozzle
        ...
        if self.nozzle.At > self.chamber_area:
            raise ValueError(
                f"The combustion chamber area {self.chamber_area} m^2 is smaller than the throat area {self.nozzle.At} m^2.")

        if self.geometry == "auto":
            # Use the system defined in Reference [1] - mostly using Eqns (4)
            # Make sure we cap the size of the converging section to the radius
            # of the combustion chamber.
            chamber_radius = (self.chamber_area / np.pi)**0.5
            theta_min = -np.pi - \
                np.arcsin((chamber_radius - self.nozzle.Rt - 1.5 * self.nozzle.Rt) / (1.5 * self.nozzle.Rt))
            ...
            self.x_curved_converging_start = 1.5 * self.nozzle.Rt * \
                np.cos(self.theta_curved_converging_start)
            self.y_curved_converging_start = 1.5 * self.nozzle.Rt * \
                np.sin(self.theta_curved_converging_start) + 1.5 * self.nozzle.Rt + self.nozzle.Rt

            # Find the gradient where the curved converging bit starts
            dxdtheta_curved_converging_start = -1.5 * self.nozzle.Rt * \
                np.sin(self.theta_curved_converging_start)
            self.dydx_curved_converging_start = -1.5 * self.nozzle.Rt * \
                np.cos(self.theta_curved_converging_start) / dxdtheta_curved_converging_start

            # Find the x-position where we reach the combustion chamber radius
            self.x_chamber_end = self.x_curved_converging_start - \
                (self.chamber_radius - self.y_curved_converging_start) / self.dydx_curved_converging_start
    ...
    def y(self, x):
        if self.geometry == "auto":
            ...
            if x < 0 and x > self.x_curved_converging_start:
                ...
                return 1.5 * self.nozzle.Rt * \
                    np.sin(theta) + 1.5 * self.nozzle.Rt + self.nozzle.Rt
            ...
            # Before the curved part of the converging section
                ...
                # Inside the converging section
                    ...
                    return np.interp(x, [self.x_chamber_end, self.x_curved_converging_start], [
                                     self.chamber_radius, self.y_curved_converging_start])

                # Outside of the engine
                else:
                    return ValueError(
                        f"x is beyond the front of the engine. You tried to input {x} but the minimum value you're allowed is {self.x_chamber_end - self.chamber_length}")
...
class CoolingJacket:
    ...
    def __init__(self, inner_wall, inlet_T, inlet_p0, thermo_coolant,
                 mdot_coolant, channel_shape="rectangle",
                 configuration="spiral", rectangle_width=None,
                 rectangle_height=None, circle_diameter=None,
...
class EngineWithCooling:
        ...
        # If we're not at the throat:
            ...
            def func_to_solve(Mach):
                return self.chamber_conditions.mdot * (self.perfect_gas.cp * self.chamber_conditions.T0)**0.5 / (
                    self.geometry.A(x) * self.chamber_conditions.p0) - bam.m_bar(Mach, self.perfect_gas.gamma)

            if x > 0:
                Mach = scipy.optimize.root_scalar(
                    func_to_solve, bracket=[1, 300], x0=1).root
            else:
                Mach = scipy.optimize.root_scalar(
                    func_to_solve, bracket=[0, 1], x0=0.5).root
    ...
    def coolant_velocity(self, x, rho_coolant):
        return self.cooling_jacket.mdot_coolant / \
            (rho_coolant * self.cooling_jacket.A(x))
    ...
    def h_gas_bartz_1(
        ...
        return (0.026 / D**0.2) * (cp_inf * mu_inf**0.2) / (Pr_inf**0.6) * \
            (rho_inf * v_inf)**0.8 * (rho_am / rho_inf) * (mu_am / mu0)**0.2
    ...
    def h_gas_bartz_2(self, mu, cp, Pr, M, A, Tw):
        ...
        """
        c_star = self.chamber_conditions.p0 * \
            self.geometry.nozzle.At / self.chamber_conditions.mdot
        ...
        sigma = (0.5 * (Tw / Tc) * (1 + (gamma - 1) / 2 * M**2) +
                 0.5)**0.68 * (1 + (gamma - 1) / 2 * M**2)**(-0.12)

        return (0.026) / (Dt**0.2) * (mu**0.2 * cp / Pr**0.6) * \
            (pc / c_star)**0.8 * (At / A)**0.9 * sigma
    ...
    def thermal_circuit(
        ...
        A_in = 2 * np.pi * r_out
        # Outer area per unit length (i.e. just the outer circumference)
        A_out = 2 * np.pi * r_in
        R_gas = 1 / (h_gas * A_in)
        R_wall = np.log(r_out / r_in) / (2 * np.pi * inner_wall.k)
        R_coolant = 1 / (h_coolant * A_out)
        q_dot = (T_gas - T_coolant) / (R_gas + R_wall +
                                       R_coolant)  # Heat flux per length
    ...
    def run_heating_analysis(
        ...
        # Run from the back end (the nozzle exit) to the front (chamber)
        discretised_x = np.linspace(
            self.geometry.x_max,
            self.geometry.x_min,
            number_of_points)
        ...
        q_Adot = np.zeros(len(discretised_x))
        ...
        for i in range(len(discretised_x)):
            ...
            h_coolant[i] = self.h_coolant(
                x, mu_coolant, k_coolant, cp_coolant, rho_coolant)
            ...
            if i == 0:
                ...
                T_coolant[i] = T_coolant[i - 1] + \
                    (q_dot[i - 1] * dx) / (self.cooling_jacket.mdot_coolant * cp_coolant)
            ...
            if h_gas_model == "standard":
                ...
                # p = rho R T - pressure is roughly uniform across the boundary
                # layer so p_inf ~= p_wall
                rho_am = p_inf / (R * T_am)
                ...
                h_gas[i] = self.h_gas_bartz_1(
                    D, cp_inf, mu_inf, Pr_inf, rho_inf, v_inf, rho_am, mu_am, mu0)
                ...
                raise AttributeError(
                    f"Could not find the h_gas_model {h_gas_model}")
    ...
    def run_stress_analysis(
        ...
        # Spent an hour wondering why the throat was cooler than the chamber
        # wall...
