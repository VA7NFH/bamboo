...
def m_bar(M, gamma):
    ...
    return gamma / (gamma - 1)**0.5 * M * (1 + M**2 * \
                    (gamma - 1) / 2)**(-0.5 * (gamma + 1) / (gamma - 1))
...
def estimate_apogee(
    ...
    def fdot(fn, t):
        ...
        if t < burn_time:
            ...
            try:
                net_force = engine.thrust(
                    p_amb) - 0.5 * density_amb * fn[1]**2 * drag_coefficient * cross_sectional_area - mass * g0
            except ValueError:
                raise ValueError(
                    f"Flow seperation occured in the nozzle at an altitude of {fn[0]/1000} km")
            acc = net_force / mass

        else:
            # Engine burnt out
            net_force = -0.5 * density_amb * \
                fn[1]**2 * drag_coefficient * cross_sectional_area - dry_mass * g0
    ...
    while reached_apogee == False:
        ...
        fnplusone = fn + (1 / 6) * (k1 + 2 * k2 + 2 * k3 +
                                    k4) * dt   # + O(dt^5) = [alt, vel]
...
def rao_theta_n(area_ratio, length_fraction=0.8):
    ...
    if length_fraction == 0.8:
        ...
        raise ValueError(
            "The length percent given does not match any of the available data.")
    ...
    if area_ratio < 3.7 or area_ratio > 47:
        ...
        # Linearly interpolate and return the result, after converting it to
        # radians.
        return np.interp(
            area_ratio,
            data["area_ratio"],
            data["theta_n"]) * np.pi / 180
...
def get_throat_area(gas, chamber_conditions):
    ...
    """
    return (chamber_conditions.mdot * (gas.cp * chamber_conditions.T0)
            ** 0.5) / (m_bar(1, gas.gamma) * chamber_conditions.p0)
...
def get_exit_area(gas, chamber_conditions, p_amb):
    ...
    return (chamber_conditions.mdot * (gas.cp * chamber_conditions.T0)
            ** 0.5) / (m_bar(Me, gas.gamma) * chamber_conditions.p0)
...
def show_conical_shape(A1, At, A2, div_half_angle=15, conv_half_angle=45):
    ...
    x = [0, (r1 - rt) / np.tan(conv_half_angle), (r1 - rt) /
         np.tan(conv_half_angle) + (r2 - rt) / np.tan(div_half_angle)]
    y_pos = [r1, rt, r2]
    y_neg = [-r1, -rt, -r2]
...
class PerfectGas:
    ...
    def __init__(self, **kwargs):
        if len(kwargs) > 2:
            raise ValueError(
                f"Gas object is overdefined. You mustn't provide more than 2 inputs when creating the Gas object. You provided {len(kwargs)}.")
            ...
            raise ValueError(
                f"Not enough inputs provided to fully define the Gas. You must provide exactly 2, but you provided only {len(kwargs)}.")
...
class Nozzle:
    ...
    def __init__(self, At, Ae, type="rao", length_fraction=0.8):
        ...
        if self.type == "rao":
            # Inflection angle (rad), as defined in [1]
            self.theta_n = rao_theta_n(self.Ae / self.At)
            # Exit angle (rad), as defined in [1]
            self.theta_e = rao_theta_e(self.Ae / self.At)

            # Page 5 of Reference [1]:
            # Inflection point x-value
            self.Nx = 0.382 * self.Rt * np.cos(self.theta_n - np.pi / 2)
            # Inflection point y-value
            self.Ny = 0.382 * self.Rt * \
                np.sin(self.theta_n - np.pi / 2) + 0.382 * self.Rt + self.Rt
            self.Ex = 0.8 * ((self.Re / self.Rt) - 1) * \
                self.Rt / np.tan(np.pi / 12)  # Exit x-value
            self.Ey = self.Re  # Exit y-value (same as Re)
            self.length = self.Ex  # Nozzle length

            # Page 2 of Reference [3]. Set up the matrix problem to get the
            # coefficients for x = ay^2 + by + c
            A = np.array([[2 * self.Ny, 1, 0],
                          [2 * self.Ey, 1, 0],
                          [self.Ny**2, self.Ny, 1]], dtype='float')

            b = np.array([1 / np.tan(self.theta_n), 1 /
                          np.tan(self.theta_e), self.Nx], dtype='float')

            self.a, self.b, self.c = np.linalg.solve(A, b)

        else:
            raise ValueError(
                f"Nozzle type '{type}' is not currently implemented. Try 'conical' or 'rao'")
    ...
    def y(self, x):
        ...
        if x < 0:
            raise ValueError(
                f"x must be greater than zero. You tried to input {x}.")

        elif self.type == "rao" and x <= self.length:
            # Circular throat section
            if x < self.Nx:
                # Take the negative, because we want an answer in the range
                # [-90 to 0 deg], but numpy gives us the one in the range [0 to
                # 180 deg]
                theta = -np.arccos(x / (0.382 * self.Rt))
                return 0.382 * self.Rt * \
                    np.sin(theta) + 0.382 * self.Rt + self.Rt

            # Parabolic section.
            else:
                # Rearranging the quadratic on page 2 of Reference [3] to solve
                # for y
                return ((4 * self.a * (x - self.c) + self.b**2)
                        ** 0.5 - self.b) / (2 * self.a)

        else:
            raise ValueError(
                f"x is beyond the end of the nozzle, which is only {self.length} m long. You tried to input {x}.")
...
class Engine:
    ...
    def M(self, x):
        ...
        # If we're not at the throat:
            ...
            def func_to_solve(Mach):
                return self.chamber_conditions.mdot * (self.perfect_gas.cp * self.chamber_conditions.T0)**0.5 / (
                    self.nozzle.A(x) * self.chamber_conditions.p0) - m_bar(Mach, self.perfect_gas.gamma)

            Mach = scipy.optimize.root_scalar(
                func_to_solve, bracket=[1, 25], x0=1).root
    ...
    def check_seperation(self, p_amb):
        ...
        seperation_pressure_ratio = 0.583 * \
            (p_amb / self.chamber_conditions.p0)**0.195

        # Seperation can't occur if there's a vacuum outside:
        if p_amb == 0:
            return False

        # Check for seperation by comparing to the lowest pressure in the
        # nozzle (which will be at exit):
        elif self.p(self.nozzle.length) / p_amb < seperation_pressure_ratio:
            def func_to_solve(x):
                # Should equal zero at the seperation point
                return self.p(x) / p_amb - seperation_pressure_ratio

            # Find the seperation position.
            return scipy.optimize.root_scalar(
                func_to_solve, bracket=[
                    0, self.nozzle.length], x0=0).root
    ...
    def thrust(self, p_amb):
        ...
        if self.check_seperation(p_amb) == False:
            ...
            return self.chamber_conditions.mdot * Me * (
                self.perfect_gas.gamma * self.perfect_gas.R * Te)**0.5 + (
                pe - p_amb) * self.nozzle.Ae  # Generic equation for rocket thrust

        else:
            raise ValueError(
                f"Seperation occured in the nozzle, at a postion {self.check_seperation(p_amb)} m downstream of the throat.")
    ...
    def optimise_for_apogee(
        ...
        # We need to calculate bounds to avoid causing flow seperation in the
        # nozzle
        # Exit area that would cause seperation at sea level.
        Ae_for_sepeation = self.seperation_Ae(
            ambiance.Atmosphere(0).pressure[0])
        ...
        def get_apogee(Ae):
            ...
            test_engine.nozzle = Nozzle(
                At,
                Ae,
                test_engine.nozzle.type,
                test_engine.nozzle.length_fraction)
            apogee_estimate = estimate_apogee(
                dry_mass,
                propellant_mass,
                test_engine,
                cross_sectional_area,
            ...
            if debug:
                print(
                    f"Area ratio = {Ae/At}, apogee = {apogee_estimate/1000} km")

            return apogee_estimate

        def func_to_minimise(Ae):
            # Just make it the negative of the apogee - so we want to minimise
            # the output of this function (scipy can only minimise, not
            # maximise)
            return -get_apogee(Ae)

        if debug:
            print(f"Starting optimisation with bounds Ae/At = {bounds/At}")

        optimum_Ae = scipy.optimize.minimize_scalar(
            func_to_minimise, bounds=bounds, method="bounded").x
        max_apogee = get_apogee(optimum_Ae)
        self.nozzle = Nozzle(
            At,
            optimum_Ae,
            self.nozzle.type,
            self.nozzle.length_fraction)

        print(
            f"Area ratio optimised with Ae/At = {self.nozzle.Ae/self.nozzle.At}, giving apogee = {max_apogee/1000} km")
